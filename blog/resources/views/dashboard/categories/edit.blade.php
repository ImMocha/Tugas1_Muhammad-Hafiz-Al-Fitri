...
<div class="col-lg-8">
    <form method="POST" action="/dashboard/categories/{{ $categories->slug }}" class="mb-5" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input required autofocus name="name" type="text" class="form-control 
            @error('name')
                is-invalid
            @enderror" id="name" value="{{ old('name', $categories->name) }}"/>
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input name="slug" value="{{ old('slug',$categories->slug) }}" required type="text" class="form-control 
            @error('slug')
            is-invalid
